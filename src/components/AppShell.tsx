...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const renderNavLinks = (onItemClick?: () => void) => (
    <>
      {nav
        .filter((item) => loading || item.roles.includes((role ?? me?.role ?? "investor") as any))
        .map((item) => {
  ...
  return (
    <div className="flex min-h-screen bg-background">
        ...
        {me && (
          <div className="m-3 rounded-lg border border-sidebar-border p-3">
            <div className="text-xs text-sidebar-foreground/60">
              {loading ? "Checking access" : (roleLabel[role ?? me.role] ?? role ?? me.role)}
            </div>
            ...
            <div className="text-[11px] text-sidebar-foreground/60 mt-1">
              {canEdit ? "Can add and edit data" : "Readonly access"}
            </div>
            <Button
              size="sm"
              variant="ghost"
              className="w-full mt-2 text-sidebar-foreground/80 hover:text-sidebar-foreground hover:bg-sidebar-accent"
              onClick={signOut}
            >
        ...
        <div className="md:hidden flex items-center justify-between px-4 py-3 border-b bg-card">
          ...
          <div className="flex items-center gap-2">
            ...
            <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
              ...
              <SheetContent side="left" className="w-[280px] p-0 bg-sidebar text-sidebar-foreground flex flex-col border-r-0">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 border-t bg-background flex items-center justify-around p-2 pb-safe z-50">
        <Link
          to="/"
          className="flex flex-col items-center gap-1 p-2 text-muted-foreground [&.active]:text-foreground"
        >
      ...
      <div>
        ...
        {description && (
          <p className="text-sm text-muted-foreground mt-1.5 max-w-2xl">
            {description}
            {lastUpdated && (
              <span className="ml-2 text-xs opacity-70">• Updated {lastUpdated}</span>
            )}
          </p>
        )}
      </div>
