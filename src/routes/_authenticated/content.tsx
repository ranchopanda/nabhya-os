...
export const Route = createFileRoute("/_authenticated/content")({
  head: () => ({
    meta: [
      { title: "Content Command Center · Nabhya OS" },
      { name: "description", content: "Every post, every platform, every metric." },
    ],
  }),
  loader: ({ context }) => {
    context.queryClient.ensureQueryData(contentPostsQuery);
  },
    ...
    <AppShell>
      <div className="px-6 lg:px-10 py-8 max-w-[1400px] mx-auto">
        <PageHeader
          ...
          description="Plan, publish, measure."
          action={
            canEdit ? (
              <ContentDialog
                trigger={
      ...
      <div className="grid md:grid-cols-3 gap-4 mb-6">
        ...
        <Card className="p-5 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="font-display text-2xl font-semibold">{published.length}</div>
          </div>
        </Card>
      </div>
