...
export const Route = createFileRoute("/_authenticated/proof")({
  ...
  loader: ({ context }) => { context.queryClient.ensureQueryData(proofDocsQuery("vault")); },
  component: ProofPage,
  ...
  return (
    <AppShell>
      <div className="px-6 lg:px-10 py-8 max-w-[1400px] mx-auto">
        <PageHeader
          eyebrow="Module 4" title="Proof Vault"
          description="The receipts — every piece of evidence that Nabhya works."
          action={canEdit ? <UploadDialog kind="vault" trigger={<Button size="sm"><Upload className="h-4 w-4" /> Upload Proof</Button>} /> : undefined}
        />
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {["All", ...PROOF_CATEGORIES].map((c) => (
          <button key={c} onClick={() => setFilter(c)}
            ...
            {c} {c !== "All" && <span className="opacity-60 ml-1">{docs.filter((d) => d.category === c).length}</span>}
          </button>
        ))}
      </div>
      <Card>
        {filtered.length === 0 ? (
          <div className="px-5 py-12 text-center text-sm text-muted-foreground">No proof uploaded yet.</div>
        ) : filtered.map((d) => (
