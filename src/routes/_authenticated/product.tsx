...
function ProductBody() {
  ...
  return (
    <>
      ...
      <Card>
        {filtered.length === 0 ? (
          <div className="px-5 py-12 text-center text-sm text-muted-foreground">No updates found.</div>
        ) : filtered.map((u) => (
          <div key={u.id} className="px-5 py-4 border-b last:border-0 grid grid-cols-12 items-center gap-4 hover:bg-accent/40 transition-colors">
            ...
            <div className="col-span-2">{u.category && <Badge variant="secondary">{u.category}</Badge>}</div>
            <div className="col-span-2 text-sm">{u.owner_name ?? "—"}</div>
            <div className="col-span-2 flex items-center justify-end gap-1">
              {canEdit && <ProductUpdateDialog update={u} trigger={<Button size="icon" variant="ghost"><Edit className="h-4 w-4" /></Button>} />}
              {isFounder && <DeleteButton table="product_updates" id={u.id} queryKey={["product_updates"]} label="product update" />}
            </div>
          </div>
        ))}
      </Card>
